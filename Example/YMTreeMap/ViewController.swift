...
class ViewController: UICollectionViewController {
    ...
    let values = TestValues_Dow30.AllValues
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: DispatchTime.now() + .seconds(3)) { [weak self] in
            ...
            self?.runBenchmark(TestValues_Dow30.self, iterations: iterations)
            self?.runBenchmark(TestValues_OneThousand.self, iterations: iterations)
    ...
    func runBenchmark(_ spec: TestValues.Type, iterations: Int) -> Void {
    ...
    func logBenchmark(_ description: String, iterations: Int, begin: UInt64, end: UInt64) {
        ...
        let diff_ms = diff / Double(NSEC_PER_MSEC)
        
        let diff_ms_str = String(format: "%f", diff_ms)
        let ops_ms_str = String(format: "%f", Double(iterations) / diff_ms)
        let ms_each_str = String(format: "%f", diff_ms / Double(iterations))
        
        print("[\(description)] Performed \(iterations) iterations in \(diff_ms_str)ms; \(ops_ms_str)/ms; \(ms_each_str)ms each")
    }
